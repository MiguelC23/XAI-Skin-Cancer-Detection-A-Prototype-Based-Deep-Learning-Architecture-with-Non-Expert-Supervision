...
def feature_portion_metric(upsampled_activation, mascaras, model,output):
    _, predicted_first = torch.max(output.data, 1)
    num_images = upsampled_activation.size(0)
    total=0
    for i in range(num_images):
        label_predicted=predicted_first[i]
        pesos_layer_for_label=model.module.last_layer.weight[label_predicted,:]
        ...
        with grad_req:
            ...
            for per in percentagens:
                ...
                for i in range(num_images):
                    ...
                    #input_no_normalized_copia[i,1, coords_pixels_remover[:, 0], coords_pixels_remover[:, 1]] = 1 if i want green
                    ...
                    r"""if(True):
                        ...
                        if(per==5):
                            ...
                            cv2.imwrite(r"C:\ACP_BM_Problem\heatmaps\\"+"heatmap_"+str(numero)+".png", heatmap_imagem)"""

                    input_alterado_normalziado= NORMALIZATION(input_no_normalized_copia)

                    
                output_p, min_distances_p, upsampled_activation_p = model(input_alterado_normalziado,mascaras)
