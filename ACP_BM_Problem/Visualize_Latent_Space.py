...
def update_and_add_tsne_plot(x, tsne, ax, label, prototypes, num_prototypes, num_classes):
    ...
    combined_data = np.vstack((x_reshaped, prototypes.view(num_prototypes, -1).cpu().numpy()))
    ...
    if(num_classes==2):
        ...
        colors = ['blue' if l == 1 else 'red' if l == 2 else 'green' if l == 3 else 'cyan' if l == 4 else 'magenta' if l == 5 else 'purple' if l == 6 else 'lime' if l == 7 else 'black' for l in label]


    # Update the scatter plot with the new t-SNE points and colors for data points
    ax.scatter(combined_tsne[:N, 0], combined_tsne[:N, 1], c=colors, marker='o', alpha=0.05) #alpha=0.05

    # Create colors for prototypes
    if(num_classes==2):
        number_colors_repeat = int(num_prototypes / 2)
        prototype_colors = ["red"] * number_colors_repeat + ["blue"] * number_colors_repeat
    elif(num_classes==8):
        number_colors_repeat = int(num_prototypes / 8)
        prototype_colors = ["black"] * number_colors_repeat + ["blue"] * number_colors_repeat + ["red"] * number_colors_repeat + ["green"] * number_colors_repeat + ["cyan"] * number_colors_repeat  + ["magenta"] * number_colors_repeat + ["purple"] * number_colors_repeat + ["lime"] * number_colors_repeat 

    # Update the scatter plot with the new t-SNE points and colors for prototypes
    ax.scatter(combined_tsne[N:, 0], combined_tsne[N:, 1], c=prototype_colors, marker='x',alpha=1)
    ...
    if(num_classes==2):
        ...
        # Create a legend with custom labels and colors
        legend_labels = {'AK': 'black', 'BCC': 'blue', "BKL":"red", "DF":"green", "MEL":"cyan", "NV":"magenta","SCC":"purple", "VASC":"lime"}
        legend_elements = [plt.Line2D([0], [0], color=legend_labels['AK'], lw=2, label=ak_label),
                        plt.Line2D([0], [0], color=legend_labels['BCC'], lw=2, label=bcc_label),
                        plt.Line2D([0], [0], color=legend_labels['BKL'], lw=2, label=bkl_label),
                        plt.Line2D([0], [0], color=legend_labels['DF'], lw=2, label=df_label),
                        plt.Line2D([0], [0], color=legend_labels['MEL'], lw=2, label=mel_label),
                        plt.Line2D([0], [0], color=legend_labels['NV'], lw=2, label=nv_label),
                        plt.Line2D([0], [0], color=legend_labels['SCC'], lw=2, label=scc_label),
                        plt.Line2D([0], [0], color=legend_labels['VASC'], lw=2, label=vasc_label),
                        ]
...
def see_cluster(model, dataloader,tsne=None, ax=None, images=True,filter_patches=True,only_max_pool_patches=True, size=(7,7)):
    ...
    for i, (image, label,ID,mask) in enumerate(dataloader):                
        ...
        with grad_req:
            ...
            reshaped_distances = distances.view(batch_size, model.module.num_prototypes, -1)  # Reshape to (BATCH SIZE,NUMBER OF PROTOTYPES, HZ*WZ) 
            ...
            # min_indices has shape torch.Size([batch_size, NUMBER OF PROTOTYPES])
    ...
    if(images==False): #each point is an image patch 
        ...
        update_and_add_tsne_plot(x_big_all, tsne, ax, label_big_all, model.module.prototype_vectors, num_prototypes=num_prototypes, num_classes=model.module.num_classes)
    elif(images==True): #each point is an image
        x_all=torch.cat(x_list, dim=0)
        label_all=torch.cat(label_list, dim=0)
        prototypes_all=model.module.prototype_vectors.cpu()
        ...
        proto_labels = [i // (model.module.num_prototypes // model.module.num_classes) for i in range(model.module.num_prototypes)] 
        #malignant prototypes have label 0 MEL is 0 and NV is 1
        # labels of the classes AK BCC BKL DF MEL NV SCC VASC respectively in this order when we have 8 classes

        contador=-1
        for prototype in prototypes_all:
            contador+=1
            ...
            for i, image in enumerate(x_all):
                if(label_all[i]==proto_labels[contador]):
        ...
        update_and_add_tsne_plot(x_all, tsne, ax, label_all, images_of_p_torch, model.module.num_prototypes,num_classes=model.module.num_classes)
