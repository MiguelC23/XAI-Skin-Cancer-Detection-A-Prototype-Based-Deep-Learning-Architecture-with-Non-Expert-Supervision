...
if __name__ == '__main__':
    ...
    k=3 #topk classes
    most_activated_prototypes=16 #chosse the same number as prototypes if you want.
    # specify the test image to be analyzed
    Test_Dataset_variable=0 # 0 ISIC 2019; 1 PH2; 2 DERM7PT
    MASKS_IN_TEST=True
    import random
    print("Choose 0 to analyze a random AK image\nChoose 1 to analyze a random BCC image\nChoose 2 to analyze a random BKL image\nChoose 3 to analyze a random DF image\nChoose 4 to analyze a random MEL image\nChoose 5 to analyze a random NV image \nChoose 6 to analyze a random SCC image\nChoose 7 to analyze a random VASC image")
    inp = ''
    valid_inputs = [0,1,2,3,4,5,6,7]
    output = {0: 'AK', 1:'BCC', 2:'BKL', 3:'DF', 4:'MEL', 5:'NV', 6:'SCC', 7:'VASC'}
    while inp not in valid_inputs:
        inp = int(input("Select one number 0,1,2,3,4,5,6,7: "))
        if inp not in valid_inputs:
            print("You must one of this 0,1,2,3,4,5,6,7")

    print(output[inp])


    if(Test_Dataset_variable==0):
        test_image_dir = r"C:\Users\migue\OneDrive\Ambiente de Trabalho\Bea_LIMPO\limpo\val\\"+output[inp] #args.test_img_dir[0]
        mask_dir=r"C:\Users\migue\OneDrive\Ambiente de Trabalho\Bea_LIMPO\limpo\val_Fine_masks\\"+output[inp]
    elif(Test_Dataset_variable==1):
        test_image_dir = r"C:\Users\migue\OneDrive\Ambiente de Trabalho\PH2_DERM7PT\PH2_test\\"+output[inp]
        mask_dir=r"C:\Users\migue\OneDrive\Ambiente de Trabalho\PH2_DERM7PT\PH2_TEST_FINE_MASKS\FINE_MASKS\\"+output[inp]
    elif(Test_Dataset_variable==2):
        test_image_dir = r"C:\Users\migue\OneDrive\Ambiente de Trabalho\PH2_DERM7PT\derm7pt_like_ISIC2019\train_val_test_224\\"+output[inp]
        mask_dir=r"C:\Users\migue\OneDrive\Ambiente de Trabalho\PH2_DERM7PT\DERM7PT_FINE_MASKS_224\\"+output[inp] 

    test_image_name=random.choice(os.listdir(test_image_dir)) 
    test_image_label = inp
    ...
    load_model_dir = r"C:\ACP_BM_Problem\NC8\resnet18\run1\\"#args.test_model_dir[0] 
    load_model_name = r"5push0.4254.pth"#args.test_model_name[0]
