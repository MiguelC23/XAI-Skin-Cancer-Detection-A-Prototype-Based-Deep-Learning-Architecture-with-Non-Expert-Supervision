...
def main():
    ...
    source_dir = r"C:\ACP_BM_Problem\NC2\resnet18\run1\IMD22.png\\" #args.local_analysis_directory[0]
    number_prototypes_in_figure=6
    ...
    for top_p in range(nrows):
        ...
        for ax in [f_axes[top_p][1]]:
            ...
            ax.get_yaxis().set_ticks([])
    #summary
    f_axes[nrows-1][4].annotate(f"This {classname_dict[int(truth)]} lesion is classified as {classname_dict[int(pred)]}.\n Because {classname_dict[int(pred)]} has the highest TC value.", **anno_opts_sum)

    save_loc1 = os.path.join(source_dir, 'visualizations_of_expl') + f'/all_classr'+str(nrows)+'.png'
...
def read_info(info_file, per_class=False):
    ...
    indst_cc_str = cc_1_line[len('proto connection to class 1:tensor('):-(len(", device='cuda:0', grad_fn=<SelectBackward>)")+2)]
