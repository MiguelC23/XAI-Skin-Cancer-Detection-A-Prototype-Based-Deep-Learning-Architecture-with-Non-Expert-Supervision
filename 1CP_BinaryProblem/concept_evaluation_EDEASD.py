...
def concept_eval(model, dataloader, size=(7,7)):
    ...
    matriz_conceitos=torch.zeros(30, int(model.module.num_prototypes/2)).cuda()
    matriz_distancias=torch.zeros(30, int(model.module.num_prototypes/2)).cuda()
            ...
            for i in range(num_imagens_ou_num_mascaras):
                ...
                for a in range(dimensao_mapa_latente):
                    for b in range(dimensao_mapa_latente):
                        distancias_patch_to_mel_prototypes=distances_mel[i,:,a,b]
                        idx_do_prototipo_mais_perto = torch.argmin(distancias_patch_to_mel_prototypes)
                        for c in range(len(conceitos_da_imagem_i)):
                            ...
                            if(mic3_mascara_nao_invertida[0,a,b]==1):
                                ...
                                matriz_distancias[numero_do_conceito,idx_do_prototipo_mais_perto]+=distancias_patch_to_mel_prototypes[idx_do_prototipo_mais_perto]
