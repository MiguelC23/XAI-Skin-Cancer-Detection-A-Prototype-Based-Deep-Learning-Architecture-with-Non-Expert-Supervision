if __name__ == '__main__':
    ...
    import test_XAI_MG3_MG4 as tnt
    from log import create_logger
    import random
    from Dataset import PH2_Dataset_or_Derm7pt_for_remove_pixels,ISIC2019_Dataset
    ...
    path_to_one_model=r"C:\1CP_BinaryProblem\NC2\resnet18\run1\20_0push0.7869.pth"
    MASKS_IN_TEST_SETS=True
    PH2=True# If true test in PH2 else test in Derm7pt
    RANDOM_HEAT_MAPS=False
    ...
    if(MASKS_IN_TEST_SETS==True):
        test_dataset = PH2_Dataset_or_Derm7pt_for_remove_pixels(test_dir, mask_dir=test_mask_dir, is_train=False,number_classes=num_classes)
    else:
        test_dataset = PH2_Dataset_or_Derm7pt_for_remove_pixels(test_dir, mask_dir=None, is_train=False,number_classes=num_classes)
    ...
    log("\n========================Performance========================\n")
    accu_test,F1_score_test,ba_test,bas_p_test,percentagens_test = tnt.test(model=ppnet_multi, dataloader=test_loader,coefs=coefs,log=log,weights=weights,heatmap_random=RANDOM_HEAT_MAPS)

    if isinstance(bas_p_test, np.ndarray):
        bas_p_test = bas_p_test.tolist()
        bas_p_test.insert(0,ba_test)
        bas_p_test=[round(num, 3) for num in bas_p_test]
    log('\t\t\ttest BA LIST: {0}\n'.format(bas_p_test))
    percentagens_test.insert(0,0)
    log('\t\t\ttest PERCENTAGES LIST: {0}\n'.format(percentagens_test))
