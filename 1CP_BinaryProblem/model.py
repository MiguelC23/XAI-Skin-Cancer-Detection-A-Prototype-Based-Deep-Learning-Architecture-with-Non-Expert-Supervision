...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
                 ...
                 LP1C_MASKED=False,Base_architecture=None,bias_value=None,Fixed_prototypes_during_training_initialized_orthogonally=False):

        super(PPNet, self).__init__()
        self.img_size = img_size
        self.prototype_shape = prototype_shape
        self.num_prototypes = prototype_shape[0]
        self.num_malignant_prototypes=int(prototype_shape[0]/2)
        self.topk_k = topk_k 
        ...
        self.last_layer = nn.Linear(self.num_prototypes//2, 1) # Only Malignant Prototypes, i.e only melanoma prototypes are used in the decision
    ...
    def forward(self, x,mascaras):
        ...
        input_melanoma = prototype_activations[:, :(self.num_prototypes//2)]
        input_melanoma_og=input_melanoma   
        input_melanoma = input_melanoma.unsqueeze(1)
        logits = self.last_layer(input_melanoma)
        ...
        return logits, min_distances, upsampled_activation,input_melanoma_og
    ...
    def prune_prototypes(self, prototypes_to_prune):
        ...
        self.last_layer.in_features = self.num_prototypes//2
        self.last_layer.out_features = 1
    ...
    def __repr__(self):
        rep = (
            'PPNet relies solely on malignant class prototypes for binary classification between Melanoma and Nevus(\n'
            '\tfeatures: {},\n'
            '\timg_size: {},\n'
            '\tprototype_shape: {},\n'
            '\tnum_malignant_prototypes: {},\n'
            '\tproto_layer_rf_info: {},\n'
            '\tnum_classes: {},\n'
            '\tepsilon: {},\n'
            '\tbase_architecture: {},\n'
    ...
    def set_last_layer_incorrect_connection(self, incorrect_strength,bias_value):
        bias_value = bias_value
        self.last_layer.bias.data.fill_(bias_value)


        input_dim = self.num_prototypes//2 #(only malignant prototypes are used for classification, Melanoma prototypes)
        incorrect_class_connection = incorrect_strength
        weights = torch.ones((input_dim))
        weights[:input_dim] = incorrect_class_connection
        self.last_layer.weight.data = weights
    ...
    def _initialize_weights(self):
        ...
        if self.last_layer_connection_weight:
            self.set_last_layer_incorrect_connection(incorrect_strength=self.last_layer_connection_weight,bias_value=self.bias_value)
        else:
            self.set_last_layer_incorrect_connection(incorrect_strength=-2,bias_value=10)
...
def construct_PPNet(base_architecture, pretrained=True, img_size=224,
                    ...
                    add_on_layers_type='bottleneck',LP1C_MASKED=False,bias_value=None,Fixed_prototypes_during_training_initialized_orthogonally=False):
    ...
    return PPNet(features=features,
                 ...
                 add_on_layers_type=add_on_layers_type,LP1C_MASKED=LP1C_MASKED,Base_architecture=base_architecture,bias_value=bias_value,Fixed_prototypes_during_training_initialized_orthogonally=Fixed_prototypes_during_training_initialized_orthogonally)
