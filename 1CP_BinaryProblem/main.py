if __name__ == '__main__':
    ...
    from settings import base_architecture,last_layer_weight,topk_k,latent_shape,experiment_run,folder_path_to_save_runs
    from settings import train_dir, test_dir, train_push_dir, \
                        train_batch_size, test_batch_size, train_push_batch_size
    from settings import train_mask_dir,online_augmentation
    from settings import test_mask_dir
    from settings import LP1C_MASKED,load_model_path
    from settings import joint_optimizer_lrs, joint_lr_step_size
    from settings import warm_optimizer_lrs
    from settings import last_layer_optimizer_lr
    from settings import coefs
    from settings import num_train_epochs, num_warm_epochs, push_start, push_epochs,diversity,number_iterations,Test_results_with_test_masks,number_of_malignant_prototype
    from settings import bias_value,Fixed_prototypes_during_training_initialized_orthogonally

    if(num_classes!=2):
        print("YOU CAN ONLU USE THIS MODEL WITH 2 CLASSES, i.e FOR A BINARY PROBLEM NOT A MULTICLASS PROBLEM.\n")
        exit(0)
    ...
    log("Using topk_k: {0}".format(topk_k)) 
    ...
    if load_model_path is not None: #Continue training without debug
        ...
        ppnet = model.construct_PPNet(base_architecture=base_architecture,
                                    ...
                                    last_layer_weight=last_layer_weight,LP1C_MASKED=LP1C_MASKED,bias_value=bias_value,
    ...
    log('Loss function coefficients:\n\tcrs_ent={};\n\tLICD={};\n\tLPAMCS={};\n'.format(coefs['crs_ent'],coefs['LICD'],coefs['LPAMCS']))

    project_name="1CP_BinaryProblem"# 1 Class Prototypes only aplicable on the Binary Problem Melanoma vs Nevus
    wandb.init(
    ...
    for epoch in range(num_train_epochs):
        ...
        if epoch < num_warm_epochs:
            ...
            accu,F1_score,ba = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=warm_optimizer,coefs=coefs, log=log,wandb=wandb,weights=weights)
        else:
            ...
            accu,F1_score,ba = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=joint_optimizer,coefs=coefs, log=log,wandb=wandb,weights=weights)
            joint_lr_scheduler.step()

        accu_test,F1_score_test,ba_test = tnt.test(model=ppnet_multi, dataloader=test_loader,coefs=coefs,log=log,wandb=wandb,weights=weights)
        ...
        log("\tParameters of last layer bias:\n{}\n".format( ppnet.last_layer.bias))
        ...
        if epoch >= push_start and epoch in push_epochs:
            ...
            if prototype_activation_function != 'linear':
                ...
                for i in range(number_iterations):
                    ...
                    accu,F1_score,ba = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=last_layer_optimizer, coefs=coefs, log=log, wandb=wandb, weights=weights, push_iterations=True)
                    
                    accu_test,F1_score_test,ba_test = tnt.test(model=ppnet_multi, dataloader=test_loader, coefs=coefs, log=log, wandb=wandb, weights=weights)
                    
                    save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + '_' + str(i) + 'push', ba=ba_test,
                                                target_ba= currbest_ba_iteration, log=log)
